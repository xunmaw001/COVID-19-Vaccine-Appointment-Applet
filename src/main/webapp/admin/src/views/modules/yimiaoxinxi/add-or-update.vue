<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="疫苗分类" prop="yimiaofenlei">
          <el-select :disabled="ro.yimiaofenlei" v-model="ruleForm.yimiaofenlei" placeholder="请选择疫苗分类">
            <el-option
                v-for="(item,index) in yimiaofenleiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="疫苗数量" prop="yimiaoshuliang">
          <el-input v-model="ruleForm.yimiaoshuliang" 
              placeholder="疫苗数量" clearable  :readonly="ro.yimiaoshuliang"></el-input>
        </el-form-item>
        <div v-else>
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="适合人群" prop="shiherenqun">
          <el-select :disabled="ro.shiherenqun" v-model="ruleForm.shiherenqun" placeholder="请选择适合人群">
            <el-option
...
export default {
  data() {
	...
	xiangqing : false,
      ...
      rules: {
          ...
          jiezhongfeiyong: [
                { required: true, message: '接种费用不能为空', trigger: 'blur' },
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/yimiaofenlei/yimiaofenlei`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.yimiaofenleiOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.xiangqing = this.ruleForm.xiangqing.replace(reg,'../../../ssmnw3e4/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
